...
_TAINT_RULE_META: Dict[str, Dict] = {
    "SAST040": {
        ...
        "debt_minutes":  240,
        "suggested_fix": "cursor.execute(sql, (param,))  # parameterised query — never format user data into SQL",
        "explanation":   "String-formatting user input into SQL lets attackers inject SQL metacharacters, altering query structure or exfiltrating data (CWE-89).",
        "confidence":    0.9,
    },
    "SAST041": {
        ...
        "debt_minutes":  240,
        "suggested_fix": "subprocess.run(['cmd', arg], shell=False, check=True)  # never pass user input to os.system",
        "explanation":   "Embedding user input in shell commands enables metacharacter injection (; | &&), allowing arbitrary command execution (CWE-78).",
        "confidence":    0.9,
    },
    "SAST042": {
        ...
        "debt_minutes":  240,
        "suggested_fix": "# Use env.get_template('name.html'); never build template strings from user input",
        "explanation":   "User-controlled template strings allow attackers to execute template expressions (e.g. {{ ''.__class__.__mro__ }}) leading to RCE (CWE-94).",
        "confidence":    0.85,
    },
    "SAST043": {
        ...
        "explanation":   "eval/exec compile and run arbitrary Python. User-controlled input reaching these calls gives full code execution with process privileges (CWE-95).",
        "confidence":    0.9,
    },
    "SAST044": {
        ...
        "debt_minutes":  120,
        "suggested_fix": "safe = pathlib.Path(base_dir).joinpath(user_input).resolve(); assert str(safe).startswith(str(base_dir))",
        "explanation":   "Unvalidated file paths allow directory traversal sequences (../../etc/passwd) to escape the intended directory (CWE-22).",
        "confidence":    0.85,
    },
    "SAST045": {
