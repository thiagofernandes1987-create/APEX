...
| Rust (56-65) | **10/10 — categoria fechada** | #56 rust-lang/rust SCA (B, 8) + CVE-2024-24576 BatBadBut (SAST AST-anchored M9.2, validação cruzada: churn=640), #57 tokio CVE-2023-22466 (SAST), #58 alacritty SCA (B, 2), #60 nushell SCA (B, 9), #61 tikv SCA (D, 33), **#62 diesel soundness fix (SAST AST-anchored M9.2: `unsafe` 2→3, churn=20 — fechado em Sprint AS)**, #63 swc SCA (E, 39), #64 actix-web SCA (D, 10), #65 tauri SCA (D, 36); `rust-lang/regex` (extra) CVE-2022-24713. **#59 serde análise nativa (degradação detectada — Sprint BF: eixo de avaliação de código do sensor sobre `serde/src/de/impls.rs`; halstead_bugs 9.7→30.0 (×3.1) e duplicate_block_count 66→208 (×3.2) de v1.0.0→v1.0.219, localizado nos 35 blocos `impl Deserialize` repetidos — degradação real quantificada)** |
...
**Total real: 100/100 repositórios numerados com pelo menos um eixo de
evidência validado** — **TODAS AS 8 CATEGORIAS FECHADAS**. Eixos:
(1) SAST CVE-anchored, (2) SCA dependência-exposição, e (3) — introduzido
na Sprint BF — **análise nativa de qualidade/degradação do sensor** (o
propósito primário do produto: avaliar código, inclusive gerado por IA).
Soma auditada das categorias: 20+20+15+10+10+10+10+5 = **100**. Sprint BF
fechou os 3 últimos (`#34 boto3`, `#59 serde`, `#93 jekyll`) rodando o
motor real do sensor sobre o código — 2 com degradação detectada e
localizada (serde: halstead_bugs ×3.1, dup ×3.2, nos 35 blocos `impl
Deserialize`; jekyll: cyclomatic ×5.6, hotspot `load_theme_configuration`
L459-486) e 1 com veredito estável validado (boto3, através de 26
versões) — ver `BF_native_analysis_last3.md`. Sprint BE
...
`redisson/pom.xml` com parsing por-bloco anti-FP). Sprint BC fechou `#94
signal-android` (SCA via `gradle/libs.versions.toml`, 53 libs Maven,
limpo).
...
> lista de "restantes" omitia 2 gaps de JS/TS (#4 nodejs/node, #12
> expressjs/express — deno é #5, não "extra"; a tabela trocara os rótulos
> #3/#5/#14). Corrigido aqui: a soma das categorias é a fonte de verdade.
> Com os 2 fechamentos de AZ (clickhouse #100, elasticsearch #71) sobre a
> base correta de 87, o total exato é **89/100**.

Trajetória (corrigida): 50/100 (Sprint AN) → 69/100 (Sprint AO) →
**74/100 (AP)**, **75 (AR)**, **76 (AS)**, **77 (AT)**, **82 (AU)**,
**85 (AV)**, **86 (AW)**, **87 (AX)**, **87 (AY — roslyn fechou mas a
contagem fora inflada em +1; valor real)**, e **89/100 (AZ)**. AU
fechou 5 repos Python e AV mais 3 (cpython #21, kafka #70,
ceph #98) via o pipeline resolve-commit(GHSA M9.3)→diff-AST (M9.2); AW
introduziu o **terceiro motor — SCA de dependência vendorizada (M9.4)**
— e fechou `#91 wordpress`; AX aplicou o M9.4 a `#95 shadowsocks-windows`
(manifesto NuGet `packages.config`); AY resolveu o Central Package
Management e fechou `#89 roslyn` com um **true-positive** (MessagePack
2.5.198, 11 CVEs). Sprint AZ fechou `#100 clickhouse` (Cargo.lock em
`rust/workspace/`, limpo) e `#71 elasticsearch` (`build.versions.toml`,
jackson-databind 2.15.0 vulnerável — true-positive), fechando a categoria
Infra (5/5). Sprint AP estendeu o eixo SAST
...
**Restam 0/100 — cobertura COMPLETA.** Os 3 que resistiam aos eixos
SAST/SCA (boto3/serde/jekyll — sem CVE nem lockfile resolvível) foram
fechados na Sprint BF pelo **terceiro eixo — a análise nativa de
qualidade/degradação do sensor** (o propósito primário do produto:
avaliar código, incl. gerado por IA). Não foi fabricação: são métricas
reais sobre código real, localizadas por módulo/linha e validadas entre
versões. Todas as 8 categorias fechadas. Eixos de
