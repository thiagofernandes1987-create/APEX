...
metadata:
  author: vercel
  version: "1.0.0"
skill_id: engineering_agentops.react_view_transitions
...
output_schema:
  - name: result
    type: string
    description: "Primary output from vercel react view transitions"
what_if_fails: >
  FALLBACK: If Vercel React View Transitions cannot complete, provide partial results with
...
Provides specialized `react-view-transitions` capability to the APEX system.

## When to Use
<!-- TODO: describe when to invoke this skill vs alternatives -->
Use this skill when working with `react-view-transitions` tasks in the `engineering_agentops` domain.
