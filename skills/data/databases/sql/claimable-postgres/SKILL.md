---
skill_id: data.databases.sql.claimable_postgres
...
input_schema:
  ...
  triggers:
  - Provision instant temporary Postgres databases via Claimable Postgres by Neon (pg
...
output_schema:
  ...
  description: 'The CLI writes to the target `.env`:
    ...
    DATABASE_URL_DIRECT=postgresql://...       # direct (use for migrations, e.g'
what_if_fails:
...
Provision instant temporary Postgres databases via Claimable Postgres by Neon (pg.new). No login or credit card
