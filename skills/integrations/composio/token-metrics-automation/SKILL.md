...
what_if_fails: >
  FALLBACK: If Token Metrics Automation cannot complete, provide partial results with
...
Stub for the `token-metrics-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `token-metrics-automation` tasks via Rube MCP (Composio).
