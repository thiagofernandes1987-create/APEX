...
toolkit: lessonspace-automation
...
what_if_fails: >
  FALLBACK: If Lessonspace Automation cannot complete, provide partial results with
...
Stub for the `lessonspace-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `lessonspace-automation` tasks via Rube MCP (Composio).
