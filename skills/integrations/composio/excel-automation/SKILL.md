...
output_schema:
  - name: result
    type: string
    description: "Primary output from Excel"
what_if_fails: >
  FALLBACK: If Excel cannot complete, provide partial results with
...
Stub for the `excel-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `excel-automation` tasks via Rube MCP (Composio).
