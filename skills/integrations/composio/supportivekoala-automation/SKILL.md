...
what_if_fails: >
  FALLBACK: If Supportivekoala Automation cannot complete, provide partial results with
...
Stub for the `supportivekoala-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `supportivekoala-automation` tasks via Rube MCP (Composio).
