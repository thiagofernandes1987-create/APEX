...
what_if_fails: >
  FALLBACK: If Short Menu Automation cannot complete, provide partial results with
...
Stub for the `short-menu-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `short-menu-automation` tasks via Rube MCP (Composio).
