...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.bouncer-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: bouncer-automation
...
what_if_fails: >
  FALLBACK: If Bouncer Automation cannot complete, provide partial results with
...
Stub for the `bouncer-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `bouncer-automation` tasks via Rube MCP (Composio).
