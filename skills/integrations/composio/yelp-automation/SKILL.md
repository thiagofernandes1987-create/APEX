...
what_if_fails: >
  FALLBACK: If Yelp Automation cannot complete, provide partial results with
...
Stub for the `yelp-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `yelp-automation` tasks via Rube MCP (Composio).
