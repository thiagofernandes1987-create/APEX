...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.all-images-ai-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: all-images-ai-automation
...
what_if_fails: >
  FALLBACK: If All Images Ai Automation cannot complete, provide partial results with
...
Stub for the `all-images-ai-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `all-images-ai-automation` tasks via Rube MCP (Composio).
