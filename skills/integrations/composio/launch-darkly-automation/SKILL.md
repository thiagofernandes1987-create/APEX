...
toolkit: launch-darkly-automation
...
what_if_fails: >
  FALLBACK: If Launchdarkly cannot complete, provide partial results with
...
Stub for the `launch-darkly-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `launch-darkly-automation` tasks via Rube MCP (Composio).
