...
what_if_fails: >
  FALLBACK: If Contentful cannot complete, provide partial results with
...
Stub for the `contentful-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `contentful-automation` tasks via Rube MCP (Composio).
