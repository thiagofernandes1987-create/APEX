...
toolkit: fraudlabs-pro-automation
...
what_if_fails: >
  FALLBACK: If Fraudlabs Pro Automation cannot complete, provide partial results with
...
Stub for the `fraudlabs-pro-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `fraudlabs-pro-automation` tasks via Rube MCP (Composio).
