...
what_if_fails: >
  FALLBACK: If Zoho_Books Automation cannot complete, provide partial results with
...
Stub for the `zoho_books-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `zoho_books-automation` tasks via Rube MCP (Composio).
