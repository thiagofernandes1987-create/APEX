...
toolkit: cloudflare-browser-rendering-automation
...
what_if_fails: >
  FALLBACK: If Cloudflare Browser Rendering Automation cannot complete, provide partial results with
...
Stub for the `cloudflare-browser-rendering-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `cloudflare-browser-rendering-automation` tasks via Rube MCP (Composio).
