...
what_if_fails: >
  FALLBACK: If Nasdaq Automation cannot complete, provide partial results with
...
Stub for the `nasdaq-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `nasdaq-automation` tasks via Rube MCP (Composio).
