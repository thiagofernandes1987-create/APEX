...
toolkit: codeinterpreter-automation
...
what_if_fails: >
  FALLBACK: If Codeinterpreter Automation cannot complete, provide partial results with
...
Stub for the `codeinterpreter-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `codeinterpreter-automation` tasks via Rube MCP (Composio).
