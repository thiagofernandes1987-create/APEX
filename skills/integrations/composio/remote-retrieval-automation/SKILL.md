...
toolkit: remote-retrieval-automation
...
what_if_fails: >
  FALLBACK: If Remote Retrieval Automation cannot complete, provide partial results with
...
Stub for the `remote-retrieval-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `remote-retrieval-automation` tasks via Rube MCP (Composio).
