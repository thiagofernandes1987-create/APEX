...
what_if_fails: >
  FALLBACK: If Minerstat Automation cannot complete, provide partial results with
...
Stub for the `minerstat-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `minerstat-automation` tasks via Rube MCP (Composio).
