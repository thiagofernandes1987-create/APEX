...
output_schema:
  - name: result
    type: string
    description: "Primary output from Ramp"
what_if_fails: >
  FALLBACK: If Ramp cannot complete, provide partial results with
...
Stub for the `ramp-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ramp-automation` tasks via Rube MCP (Composio).
