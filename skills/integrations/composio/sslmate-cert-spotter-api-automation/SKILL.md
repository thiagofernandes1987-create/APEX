...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.sslmate-cert-spotter-api-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: sslmate-cert-spotter-api-automation
...
what_if_fails: >
  FALLBACK: If Sslmate Cert Spotter Api Automation cannot complete, provide partial results with
...
Stub for the `sslmate-cert-spotter-api-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `sslmate-cert-spotter-api-automation` tasks via Rube MCP (Composio).
