...
toolkit: phantombuster-automation
...
what_if_fails: >
  FALLBACK: If Phantombuster cannot complete, provide partial results with
...
Stub for the `phantombuster-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `phantombuster-automation` tasks via Rube MCP (Composio).
