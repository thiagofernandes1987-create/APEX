...
toolkit: iqair-airvisual-automation
...
what_if_fails: >
  FALLBACK: If Iqair Airvisual Automation cannot complete, provide partial results with
...
Stub for the `iqair-airvisual-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `iqair-airvisual-automation` tasks via Rube MCP (Composio).
