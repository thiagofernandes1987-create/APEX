...
toolkit: campaign-cleaner-automation
...
what_if_fails: >
  FALLBACK: If Campaign Cleaner Automation cannot complete, provide partial results with
...
Stub for the `campaign-cleaner-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `campaign-cleaner-automation` tasks via Rube MCP (Composio).
