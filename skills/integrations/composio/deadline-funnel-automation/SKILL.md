...
toolkit: deadline-funnel-automation
...
what_if_fails: >
  FALLBACK: If Deadline Funnel Automation cannot complete, provide partial results with
...
Stub for the `deadline-funnel-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `deadline-funnel-automation` tasks via Rube MCP (Composio).
