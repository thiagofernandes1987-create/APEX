...
what_if_fails: >
  FALLBACK: If Logo Dev Automation cannot complete, provide partial results with
...
Stub for the `logo-dev-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `logo-dev-automation` tasks via Rube MCP (Composio).
