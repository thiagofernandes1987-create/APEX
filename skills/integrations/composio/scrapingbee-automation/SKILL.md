...
toolkit: scrapingbee-automation
...
what_if_fails: >
  FALLBACK: If Scrapingbee Automation cannot complete, provide partial results with
...
Stub for the `scrapingbee-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `scrapingbee-automation` tasks via Rube MCP (Composio).
