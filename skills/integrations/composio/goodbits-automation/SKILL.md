...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.goodbits-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: goodbits-automation
...
what_if_fails: >
  FALLBACK: If Goodbits Automation cannot complete, provide partial results with
...
Stub for the `goodbits-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `goodbits-automation` tasks via Rube MCP (Composio).
