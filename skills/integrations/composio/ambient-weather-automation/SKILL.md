...
toolkit: ambient-weather-automation
...
what_if_fails: >
  FALLBACK: If Ambient Weather Automation cannot complete, provide partial results with
...
Stub for the `ambient-weather-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ambient-weather-automation` tasks via Rube MCP (Composio).
