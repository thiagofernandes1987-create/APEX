...
toolkit: turbot-pipes-automation
...
what_if_fails: >
  FALLBACK: If Turbot Pipes Automation cannot complete, provide partial results with
...
Stub for the `turbot-pipes-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `turbot-pipes-automation` tasks via Rube MCP (Composio).
