...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.apitemplate-io-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: apitemplate-io-automation
...
what_if_fails: >
  FALLBACK: If Apitemplate Io Automation cannot complete, provide partial results with
...
Stub for the `apitemplate-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `apitemplate-io-automation` tasks via Rube MCP (Composio).
