...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.zoho_bigin-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: zoho_bigin-automation
...
what_if_fails: >
  FALLBACK: If Zoho_Bigin Automation cannot complete, provide partial results with
...
Stub for the `zoho_bigin-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `zoho_bigin-automation` tasks via Rube MCP (Composio).
