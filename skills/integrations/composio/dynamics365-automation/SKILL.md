...
toolkit: dynamics365-automation
...
output_schema:
  - name: result
    type: string
    description: "Primary output from Dynamics"
what_if_fails: >
  FALLBACK: If Dynamics cannot complete, provide partial results with
...
Stub for the `dynamics365-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `dynamics365-automation` tasks via Rube MCP (Composio).
