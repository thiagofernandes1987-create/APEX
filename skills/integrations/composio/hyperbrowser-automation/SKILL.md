...
toolkit: hyperbrowser-automation
...
what_if_fails: >
  FALLBACK: If Hyperbrowser Automation cannot complete, provide partial results with
...
Stub for the `hyperbrowser-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `hyperbrowser-automation` tasks via Rube MCP (Composio).
