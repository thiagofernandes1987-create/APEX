...
what_if_fails: >
  FALLBACK: If Rootly Automation cannot complete, provide partial results with
...
Stub for the `rootly-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `rootly-automation` tasks via Rube MCP (Composio).
