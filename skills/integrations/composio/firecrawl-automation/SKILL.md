...
what_if_fails: >
  FALLBACK: If Firecrawl cannot complete, provide partial results with
...
Stub for the `firecrawl-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `firecrawl-automation` tasks via Rube MCP (Composio).
