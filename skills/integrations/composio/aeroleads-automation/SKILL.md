...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.aeroleads-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: aeroleads-automation
...
what_if_fails: >
  FALLBACK: If Aeroleads Automation cannot complete, provide partial results with
...
Stub for the `aeroleads-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `aeroleads-automation` tasks via Rube MCP (Composio).
