...
what_if_fails: >
  FALLBACK: If Uploadcare cannot complete, provide partial results with
...
Stub for the `uploadcare-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `uploadcare-automation` tasks via Rube MCP (Composio).
