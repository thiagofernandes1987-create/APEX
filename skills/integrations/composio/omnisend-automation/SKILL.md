...
what_if_fails: >
  FALLBACK: If Omnisend cannot complete, provide partial results with
...
Stub for the `omnisend-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `omnisend-automation` tasks via Rube MCP (Composio).
