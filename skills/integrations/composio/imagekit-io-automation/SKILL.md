...
toolkit: imagekit-io-automation
...
what_if_fails: >
  FALLBACK: If Imagekit Io Automation cannot complete, provide partial results with
...
Stub for the `imagekit-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `imagekit-io-automation` tasks via Rube MCP (Composio).
