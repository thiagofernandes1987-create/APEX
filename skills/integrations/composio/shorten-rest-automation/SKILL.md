...
toolkit: shorten-rest-automation
...
what_if_fails: >
  FALLBACK: If Shorten Rest Automation cannot complete, provide partial results with
...
Stub for the `shorten-rest-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `shorten-rest-automation` tasks via Rube MCP (Composio).
