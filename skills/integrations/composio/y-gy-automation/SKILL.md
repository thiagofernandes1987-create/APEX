...
what_if_fails: >
  FALLBACK: If Y Gy Automation cannot complete, provide partial results with
...
Stub for the `y-gy-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `y-gy-automation` tasks via Rube MCP (Composio).
