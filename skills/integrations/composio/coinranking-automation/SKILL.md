...
toolkit: coinranking-automation
...
what_if_fails: >
  FALLBACK: If Coinranking Automation cannot complete, provide partial results with
...
Stub for the `coinranking-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `coinranking-automation` tasks via Rube MCP (Composio).
