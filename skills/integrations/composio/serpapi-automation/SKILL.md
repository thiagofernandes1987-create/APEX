...
what_if_fails: >
  FALLBACK: If Serpapi Automation cannot complete, provide partial results with
...
Stub for the `serpapi-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `serpapi-automation` tasks via Rube MCP (Composio).
