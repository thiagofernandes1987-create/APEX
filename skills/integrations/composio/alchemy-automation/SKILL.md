...
what_if_fails: >
  FALLBACK: If Alchemy Automation cannot complete, provide partial results with
...
Stub for the `alchemy-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `alchemy-automation` tasks via Rube MCP (Composio).
