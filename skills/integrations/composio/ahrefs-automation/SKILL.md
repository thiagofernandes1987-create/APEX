...
output_schema:
  - name: report
    type: string
    description: "Analysis report or summary from Ahrefs"
what_if_fails: >
  FALLBACK: If Ahrefs cannot complete, provide partial results with
...
Stub for the `ahrefs-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ahrefs-automation` tasks via Rube MCP (Composio).
