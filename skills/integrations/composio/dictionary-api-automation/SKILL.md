...
toolkit: dictionary-api-automation
...
what_if_fails: >
  FALLBACK: If Dictionary Api Automation cannot complete, provide partial results with
...
Stub for the `dictionary-api-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `dictionary-api-automation` tasks via Rube MCP (Composio).
