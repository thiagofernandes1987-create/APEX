...
toolkit: better-stack-automation
...
what_if_fails: >
  FALLBACK: If Better Stack Automation cannot complete, provide partial results with
...
Stub for the `better-stack-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `better-stack-automation` tasks via Rube MCP (Composio).
