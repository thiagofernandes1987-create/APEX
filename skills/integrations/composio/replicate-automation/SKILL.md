...
what_if_fails: >
  FALLBACK: If Replicate cannot complete, provide partial results with
...
Stub for the `replicate-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `replicate-automation` tasks via Rube MCP (Composio).
