...
requires:
  mcp:
    - rube
executor: HYBRID
skill_id: integrations.composio.gorgias-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: gorgias-automation
...
what_if_fails: >
  FALLBACK: If Gorgias cannot complete, provide partial results with
...
Stub for the `gorgias-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `gorgias-automation` tasks via Rube MCP (Composio).
