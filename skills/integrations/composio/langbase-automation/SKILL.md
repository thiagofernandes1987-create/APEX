...
what_if_fails: >
  FALLBACK: If Langbase Automation cannot complete, provide partial results with
...
Stub for the `langbase-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `langbase-automation` tasks via Rube MCP (Composio).
