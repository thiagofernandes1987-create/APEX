...
toolkit: conversion-tools-automation
...
what_if_fails: >
  FALLBACK: If Conversion Tools Automation cannot complete, provide partial results with
...
Stub for the `conversion-tools-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `conversion-tools-automation` tasks via Rube MCP (Composio).
