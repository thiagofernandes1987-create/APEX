...
toolkit: cdr-platform-automation
...
what_if_fails: >
  FALLBACK: If Cdr Platform Automation cannot complete, provide partial results with
...
Stub for the `cdr-platform-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `cdr-platform-automation` tasks via Rube MCP (Composio).
