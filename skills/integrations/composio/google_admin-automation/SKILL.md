...
toolkit: google_admin-automation
...
what_if_fails: >
  FALLBACK: If Google_Admin Automation cannot complete, provide partial results with
...
Stub for the `google_admin-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `google_admin-automation` tasks via Rube MCP (Composio).
