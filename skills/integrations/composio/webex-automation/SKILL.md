...
what_if_fails: >
  FALLBACK: If Webex cannot complete, provide partial results with
...
Stub for the `webex-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `webex-automation` tasks via Rube MCP (Composio).
