...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.wati-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: wati-automation
...
what_if_fails: >
  FALLBACK: If Wati Automation cannot complete, provide partial results with
...
Stub for the `wati-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `wati-automation` tasks via Rube MCP (Composio).
