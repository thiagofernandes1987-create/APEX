...
toolkit: btcpay-server-automation
...
what_if_fails: >
  FALLBACK: If Btcpay Server Automation cannot complete, provide partial results with
...
Stub for the `btcpay-server-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `btcpay-server-automation` tasks via Rube MCP (Composio).
