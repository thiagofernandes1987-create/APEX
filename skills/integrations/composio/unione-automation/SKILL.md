...
what_if_fails: >
  FALLBACK: If Unione Automation cannot complete, provide partial results with
...
Stub for the `unione-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `unione-automation` tasks via Rube MCP (Composio).
