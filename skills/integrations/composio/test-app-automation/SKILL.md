...
what_if_fails: >
  FALLBACK: If Test App Automation cannot complete, provide partial results with
...
Stub for the `test-app-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `test-app-automation` tasks via Rube MCP (Composio).
