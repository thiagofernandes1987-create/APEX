...
toolkit: happy-scribe-automation
...
what_if_fails: >
  FALLBACK: If Happy Scribe Automation cannot complete, provide partial results with
...
Stub for the `happy-scribe-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `happy-scribe-automation` tasks via Rube MCP (Composio).
