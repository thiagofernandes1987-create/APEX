...
what_if_fails: >
  FALLBACK: If Dadata Ru Automation cannot complete, provide partial results with
...
Stub for the `dadata-ru-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `dadata-ru-automation` tasks via Rube MCP (Composio).
