...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.text-to-pdf-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: text-to-pdf-automation
...
what_if_fails: >
  FALLBACK: If Text To Pdf Automation cannot complete, provide partial results with
...
Stub for the `text-to-pdf-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `text-to-pdf-automation` tasks via Rube MCP (Composio).
