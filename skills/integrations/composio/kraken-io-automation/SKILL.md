...
what_if_fails: >
  FALLBACK: If Kraken Io Automation cannot complete, provide partial results with
...
Stub for the `kraken-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `kraken-io-automation` tasks via Rube MCP (Composio).
