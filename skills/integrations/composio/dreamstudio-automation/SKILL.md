...
toolkit: dreamstudio-automation
...
what_if_fails: >
  FALLBACK: If Dreamstudio Automation cannot complete, provide partial results with
...
Stub for the `dreamstudio-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `dreamstudio-automation` tasks via Rube MCP (Composio).
