...
what_if_fails: >
  FALLBACK: If Salesmate Automation cannot complete, provide partial results with
...
Stub for the `salesmate-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `salesmate-automation` tasks via Rube MCP (Composio).
