...
toolkit: browserless-automation
...
what_if_fails: >
  FALLBACK: If Browserless Automation cannot complete, provide partial results with
...
Stub for the `browserless-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `browserless-automation` tasks via Rube MCP (Composio).
