...
toolkit: uptimerobot-automation
...
what_if_fails: >
  FALLBACK: If Uptimerobot Automation cannot complete, provide partial results with
...
Stub for the `uptimerobot-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `uptimerobot-automation` tasks via Rube MCP (Composio).
