...
what_if_fails: >
  FALLBACK: If Gong cannot complete, provide partial results with
...
Stub for the `gong-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `gong-automation` tasks via Rube MCP (Composio).
