...
toolkit: graphhopper-automation
...
what_if_fails: >
  FALLBACK: If Graphhopper Automation cannot complete, provide partial results with
...
Stub for the `graphhopper-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `graphhopper-automation` tasks via Rube MCP (Composio).
