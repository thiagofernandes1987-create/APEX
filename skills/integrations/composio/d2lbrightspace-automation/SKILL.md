...
Stub for the `d2lbrightspace-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `d2lbrightspace-automation` tasks via Rube MCP (Composio).
