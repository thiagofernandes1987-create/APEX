...
toolkit: emailoctopus-automation
...
what_if_fails: >
  FALLBACK: If Emailoctopus Automation cannot complete, provide partial results with
...
Stub for the `emailoctopus-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `emailoctopus-automation` tasks via Rube MCP (Composio).
