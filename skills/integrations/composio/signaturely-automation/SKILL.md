...
toolkit: signaturely-automation
...
what_if_fails: >
  FALLBACK: If Signaturely Automation cannot complete, provide partial results with
...
Stub for the `signaturely-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `signaturely-automation` tasks via Rube MCP (Composio).
