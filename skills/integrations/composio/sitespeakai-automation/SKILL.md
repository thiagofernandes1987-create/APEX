...
toolkit: sitespeakai-automation
...
what_if_fails: >
  FALLBACK: If Sitespeakai Automation cannot complete, provide partial results with
...
Stub for the `sitespeakai-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `sitespeakai-automation` tasks via Rube MCP (Composio).
