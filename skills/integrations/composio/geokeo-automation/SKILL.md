...
what_if_fails: >
  FALLBACK: If Geokeo Automation cannot complete, provide partial results with
...
Stub for the `geokeo-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `geokeo-automation` tasks via Rube MCP (Composio).
