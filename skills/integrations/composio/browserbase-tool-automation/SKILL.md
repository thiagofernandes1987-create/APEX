...
toolkit: browserbase-tool-automation
...
what_if_fails: >
  FALLBACK: If Browserbase Tool Automation cannot complete, provide partial results with
...
Stub for the `browserbase-tool-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `browserbase-tool-automation` tasks via Rube MCP (Composio).
