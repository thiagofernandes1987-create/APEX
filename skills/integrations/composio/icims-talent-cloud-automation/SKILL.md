...
toolkit: icims-talent-cloud-automation
...
what_if_fails: >
  FALLBACK: If Icims Talent Cloud Automation cannot complete, provide partial results with
...
Stub for the `icims-talent-cloud-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `icims-talent-cloud-automation` tasks via Rube MCP (Composio).
