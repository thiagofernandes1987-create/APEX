...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.respond-io-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: respond-io-automation
...
what_if_fails: >
  FALLBACK: If Respond Io Automation cannot complete, provide partial results with
...
Stub for the `respond-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `respond-io-automation` tasks via Rube MCP (Composio).
