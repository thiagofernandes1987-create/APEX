...
what_if_fails: >
  FALLBACK: If Blackboard Automation cannot complete, provide partial results with
...
Stub for the `blackboard-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `blackboard-automation` tasks via Rube MCP (Composio).
