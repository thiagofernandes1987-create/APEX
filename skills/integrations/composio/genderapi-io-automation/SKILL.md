...
toolkit: genderapi-io-automation
...
what_if_fails: >
  FALLBACK: If Genderapi Io Automation cannot complete, provide partial results with
...
Stub for the `genderapi-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `genderapi-io-automation` tasks via Rube MCP (Composio).
