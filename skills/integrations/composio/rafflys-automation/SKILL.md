...
what_if_fails: >
  FALLBACK: If Rafflys Automation cannot complete, provide partial results with
...
Stub for the `rafflys-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `rafflys-automation` tasks via Rube MCP (Composio).
