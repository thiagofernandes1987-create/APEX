...
toolkit: simple-analytics-automation
...
what_if_fails: >
  FALLBACK: If Simple Analytics Automation cannot complete, provide partial results with
...
Stub for the `simple-analytics-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `simple-analytics-automation` tasks via Rube MCP (Composio).
