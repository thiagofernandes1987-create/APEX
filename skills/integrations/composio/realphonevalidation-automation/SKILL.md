...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.realphonevalidation-automation
status: ADOPTED
security: {level: standard, pii: true, approval_required: false}
extends: integrations.composio.meta
toolkit: realphonevalidation-automation
...
what_if_fails: >
  FALLBACK: If Realphonevalidation Automation cannot complete, provide partial results with
...
Stub for the `realphonevalidation-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `realphonevalidation-automation` tasks via Rube MCP (Composio).
