...
toolkit: alpha-vantage-automation
...
what_if_fails: >
  FALLBACK: If Alpha Vantage Automation cannot complete, provide partial results with
...
Stub for the `alpha-vantage-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `alpha-vantage-automation` tasks via Rube MCP (Composio).
