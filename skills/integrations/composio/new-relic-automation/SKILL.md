...
what_if_fails: >
  FALLBACK: If New cannot complete, provide partial results with
...
Stub for the `new-relic-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `new-relic-automation` tasks via Rube MCP (Composio).
