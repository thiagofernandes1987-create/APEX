...
what_if_fails: >
  FALLBACK: If Prismic cannot complete, provide partial results with
...
Stub for the `prismic-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `prismic-automation` tasks via Rube MCP (Composio).
