...
what_if_fails: >
  FALLBACK: If Re Amaze Automation cannot complete, provide partial results with
...
Stub for the `re-amaze-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `re-amaze-automation` tasks via Rube MCP (Composio).
