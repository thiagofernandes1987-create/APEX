...
toolkit: owl-protocol-automation
...
what_if_fails: >
  FALLBACK: If Owl Protocol Automation cannot complete, provide partial results with
...
Stub for the `owl-protocol-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `owl-protocol-automation` tasks via Rube MCP (Composio).
