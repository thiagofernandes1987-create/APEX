...
what_if_fails: >
  FALLBACK: If Facebook cannot complete, provide partial results with
...
Stub for the `facebook-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `facebook-automation` tasks via Rube MCP (Composio).
