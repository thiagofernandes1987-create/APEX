...
toolkit: beaconchain-automation
...
what_if_fails: >
  FALLBACK: If Beaconchain Automation cannot complete, provide partial results with
...
Stub for the `beaconchain-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `beaconchain-automation` tasks via Rube MCP (Composio).
