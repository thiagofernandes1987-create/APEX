...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.nano-nets-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: nano-nets-automation
...
what_if_fails: >
  FALLBACK: If Nano Nets Automation cannot complete, provide partial results with
...
Stub for the `nano-nets-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `nano-nets-automation` tasks via Rube MCP (Composio).
