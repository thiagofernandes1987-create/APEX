...
toolkit: survey-monkey-automation
...
what_if_fails: >
  FALLBACK: If Surveymonkey cannot complete, provide partial results with
...
Stub for the `survey-monkey-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `survey-monkey-automation` tasks via Rube MCP (Composio).
