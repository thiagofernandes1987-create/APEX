...
output_schema:
  - name: result
    type: string
    description: "Primary output from NetSuite"
what_if_fails: >
  FALLBACK: If Netsuite cannot complete, provide partial results with
...
Stub for the `netsuite-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `netsuite-automation` tasks via Rube MCP (Composio).
