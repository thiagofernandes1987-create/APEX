...
what_if_fails: >
  FALLBACK: If Boloforms Automation cannot complete, provide partial results with
...
Stub for the `boloforms-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `boloforms-automation` tasks via Rube MCP (Composio).
