...
toolkit: lemon-squeezy-automation
...
what_if_fails: >
  FALLBACK: If Lemon cannot complete, provide partial results with
...
Stub for the `lemon-squeezy-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `lemon-squeezy-automation` tasks via Rube MCP (Composio).
