...
what_if_fails: >
  FALLBACK: If Reply Io Automation cannot complete, provide partial results with
...
Stub for the `reply-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `reply-io-automation` tasks via Rube MCP (Composio).
