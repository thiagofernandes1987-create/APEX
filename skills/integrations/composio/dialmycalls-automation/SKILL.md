...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.dialmycalls-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: dialmycalls-automation
...
what_if_fails: >
  FALLBACK: If Dialmycalls Automation cannot complete, provide partial results with
...
Stub for the `dialmycalls-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `dialmycalls-automation` tasks via Rube MCP (Composio).
