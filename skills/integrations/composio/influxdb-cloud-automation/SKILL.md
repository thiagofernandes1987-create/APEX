...
toolkit: influxdb-cloud-automation
...
what_if_fails: >
  FALLBACK: If Influxdb Cloud Automation cannot complete, provide partial results with
...
Stub for the `influxdb-cloud-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `influxdb-cloud-automation` tasks via Rube MCP (Composio).
