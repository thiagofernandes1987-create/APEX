...
what_if_fails: >
  FALLBACK: If Botsonic Automation cannot complete, provide partial results with
...
Stub for the `botsonic-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `botsonic-automation` tasks via Rube MCP (Composio).
