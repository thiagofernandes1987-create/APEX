...
what_if_fails: >
  FALLBACK: If Weathermap Automation cannot complete, provide partial results with
...
Stub for the `weathermap-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `weathermap-automation` tasks via Rube MCP (Composio).
