...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.brex-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: brex-automation
...
what_if_fails: >
  FALLBACK: If Brex Automation cannot complete, provide partial results with
...
Stub for the `brex-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `brex-automation` tasks via Rube MCP (Composio).
