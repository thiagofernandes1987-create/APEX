...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.capsule_crm-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: capsule_crm-automation
...
what_if_fails: >
  FALLBACK: If Capsule_Crm Automation cannot complete, provide partial results with
...
Stub for the `capsule_crm-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `capsule_crm-automation` tasks via Rube MCP (Composio).
