...
what_if_fails: >
  FALLBACK: If Heygen cannot complete, provide partial results with
...
Stub for the `heygen-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `heygen-automation` tasks via Rube MCP (Composio).
