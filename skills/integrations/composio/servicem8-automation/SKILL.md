...
what_if_fails: >
  FALLBACK: If Servicem8 Automation cannot complete, provide partial results with
...
Stub for the `servicem8-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `servicem8-automation` tasks via Rube MCP (Composio).
