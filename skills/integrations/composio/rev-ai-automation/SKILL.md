...
what_if_fails: >
  FALLBACK: If Rev Ai Automation cannot complete, provide partial results with
...
Stub for the `rev-ai-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `rev-ai-automation` tasks via Rube MCP (Composio).
