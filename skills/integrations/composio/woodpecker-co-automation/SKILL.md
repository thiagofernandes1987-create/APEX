...
what_if_fails: >
  FALLBACK: If Woodpecker Co Automation cannot complete, provide partial results with
...
Stub for the `woodpecker-co-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `woodpecker-co-automation` tasks via Rube MCP (Composio).
