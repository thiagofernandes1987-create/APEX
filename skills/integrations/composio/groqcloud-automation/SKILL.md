...
what_if_fails: >
  FALLBACK: If Groqcloud cannot complete, provide partial results with
...
Stub for the `groqcloud-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `groqcloud-automation` tasks via Rube MCP (Composio).
