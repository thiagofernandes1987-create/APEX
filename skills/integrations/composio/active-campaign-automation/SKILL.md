...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.active-campaign-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: active-campaign-automation
...
what_if_fails: >
  FALLBACK: If Active Campaign Automation cannot complete, provide partial results with
...
Stub for the `active-campaign-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `active-campaign-automation` tasks via Rube MCP (Composio).
