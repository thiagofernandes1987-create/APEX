...
what_if_fails: >
  FALLBACK: If Customer.Io cannot complete, provide partial results with
...
Stub for the `customerio-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `customerio-automation` tasks via Rube MCP (Composio).
