...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.fillout-forms-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: fillout-forms-automation
...
what_if_fails: >
  FALLBACK: If Fillout Forms Automation cannot complete, provide partial results with
...
Stub for the `fillout-forms-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `fillout-forms-automation` tasks via Rube MCP (Composio).
