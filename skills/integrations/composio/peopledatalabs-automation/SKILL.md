...
toolkit: peopledatalabs-automation
...
what_if_fails: >
  FALLBACK: If Peopledatalabs Automation cannot complete, provide partial results with
...
Stub for the `peopledatalabs-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `peopledatalabs-automation` tasks via Rube MCP (Composio).
