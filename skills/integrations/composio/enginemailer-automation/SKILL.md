...
toolkit: enginemailer-automation
...
what_if_fails: >
  FALLBACK: If Enginemailer Automation cannot complete, provide partial results with
...
Stub for the `enginemailer-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `enginemailer-automation` tasks via Rube MCP (Composio).
