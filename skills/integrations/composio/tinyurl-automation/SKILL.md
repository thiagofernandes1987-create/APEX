...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.tinyurl-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: tinyurl-automation
...
what_if_fails: >
  FALLBACK: If Tinyurl Automation cannot complete, provide partial results with
...
Stub for the `tinyurl-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `tinyurl-automation` tasks via Rube MCP (Composio).
