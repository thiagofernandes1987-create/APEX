...
what_if_fails: >
  FALLBACK: If Telnyx Automation cannot complete, provide partial results with
...
Stub for the `telnyx-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `telnyx-automation` tasks via Rube MCP (Composio).
