...
what_if_fails: >
  FALLBACK: If Elevenlabs cannot complete, provide partial results with
...
Stub for the `elevenlabs-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `elevenlabs-automation` tasks via Rube MCP (Composio).
