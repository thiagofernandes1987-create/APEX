...
toolkit: ticketmaster-automation
...
what_if_fails: >
  FALLBACK: If Ticketmaster Automation cannot complete, provide partial results with
...
Stub for the `ticketmaster-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ticketmaster-automation` tasks via Rube MCP (Composio).
