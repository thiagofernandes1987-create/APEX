...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.backendless-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: backendless-automation
...
what_if_fails: >
  FALLBACK: If Backendless Automation cannot complete, provide partial results with
...
Stub for the `backendless-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `backendless-automation` tasks via Rube MCP (Composio).
