...
what_if_fails: >
  FALLBACK: If Hunter cannot complete, provide partial results with
...
Stub for the `hunter-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `hunter-automation` tasks via Rube MCP (Composio).
