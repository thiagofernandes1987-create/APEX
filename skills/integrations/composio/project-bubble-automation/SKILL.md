...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.project-bubble-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: project-bubble-automation
...
what_if_fails: >
  FALLBACK: If Project Bubble Automation cannot complete, provide partial results with
...
Stub for the `project-bubble-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `project-bubble-automation` tasks via Rube MCP (Composio).
