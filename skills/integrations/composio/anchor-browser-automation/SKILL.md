...
what_if_fails: >
  FALLBACK: If Anchor Browser Automation cannot complete, provide partial results with
...
Stub for the `anchor-browser-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `anchor-browser-automation` tasks via Rube MCP (Composio).
