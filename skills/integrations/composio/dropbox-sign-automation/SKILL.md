...
what_if_fails: >
  FALLBACK: If Dropbox Sign Automation cannot complete, provide partial results with
...
Stub for the `dropbox-sign-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `dropbox-sign-automation` tasks via Rube MCP (Composio).
