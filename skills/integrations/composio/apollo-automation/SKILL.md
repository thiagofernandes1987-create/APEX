...
requires:
  mcp:
    - rube
executor: HYBRID
skill_id: integrations.composio.apollo-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: apollo-automation
...
what_if_fails: >
  FALLBACK: If Apollo cannot complete, provide partial results with
...
Stub for the `apollo-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `apollo-automation` tasks via Rube MCP (Composio).
