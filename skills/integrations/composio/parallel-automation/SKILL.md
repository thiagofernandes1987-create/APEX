...
what_if_fails: >
  FALLBACK: If Parallel Automation cannot complete, provide partial results with
...
Stub for the `parallel-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `parallel-automation` tasks via Rube MCP (Composio).
