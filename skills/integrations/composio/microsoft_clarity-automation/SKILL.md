...
toolkit: microsoft_clarity-automation
...
what_if_fails: >
  FALLBACK: If Microsoft_Clarity Automation cannot complete, provide partial results with
...
Stub for the `microsoft_clarity-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `microsoft_clarity-automation` tasks via Rube MCP (Composio).
