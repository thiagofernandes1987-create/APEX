...
what_if_fails: >
  FALLBACK: If Snowflake cannot complete, provide partial results with
...
Stub for the `snowflake-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `snowflake-automation` tasks via Rube MCP (Composio).
