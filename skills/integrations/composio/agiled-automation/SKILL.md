...
what_if_fails: >
  FALLBACK: If Agiled Automation cannot complete, provide partial results with
...
Stub for the `agiled-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `agiled-automation` tasks via Rube MCP (Composio).
