...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.googleslides-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: googleslides-automation
...
what_if_fails: >
  FALLBACK: If Googleslides Automation cannot complete, provide partial results with
...
Stub for the `googleslides-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `googleslides-automation` tasks via Rube MCP (Composio).
