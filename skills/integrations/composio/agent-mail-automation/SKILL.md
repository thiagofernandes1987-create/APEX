...
what_if_fails: >
  FALLBACK: If Agent Mail Automation cannot complete, provide partial results with
...
Stub for the `agent-mail-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `agent-mail-automation` tasks via Rube MCP (Composio).
