...
output_schema:
  - name: result
    type: string
    description: "Primary output from FreshBooks"
what_if_fails: >
  FALLBACK: If Freshbooks cannot complete, provide partial results with
...
Stub for the `freshbooks-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `freshbooks-automation` tasks via Rube MCP (Composio).
