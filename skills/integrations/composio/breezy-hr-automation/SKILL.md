...
what_if_fails: >
  FALLBACK: If Breezy Hr Automation cannot complete, provide partial results with
...
Stub for the `breezy-hr-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `breezy-hr-automation` tasks via Rube MCP (Composio).
