...
toolkit: storerocket-automation
...
what_if_fails: >
  FALLBACK: If Storerocket Automation cannot complete, provide partial results with
...
Stub for the `storerocket-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `storerocket-automation` tasks via Rube MCP (Composio).
