...
what_if_fails: >
  FALLBACK: If Shortcut cannot complete, provide partial results with
...
Stub for the `shortcut-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `shortcut-automation` tasks via Rube MCP (Composio).
