...
toolkit: googlesuper-automation
...
what_if_fails: >
  FALLBACK: If Googlesuper Automation cannot complete, provide partial results with
...
Stub for the `googlesuper-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `googlesuper-automation` tasks via Rube MCP (Composio).
