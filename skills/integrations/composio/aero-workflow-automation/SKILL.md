...
what_if_fails: >
  FALLBACK: If Aero Workflow Automation cannot complete, provide partial results with
...
Stub for the `aero-workflow-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `aero-workflow-automation` tasks via Rube MCP (Composio).
