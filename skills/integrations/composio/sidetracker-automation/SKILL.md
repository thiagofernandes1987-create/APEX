...
toolkit: sidetracker-automation
...
what_if_fails: >
  FALLBACK: If Sidetracker Automation cannot complete, provide partial results with
...
Stub for the `sidetracker-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `sidetracker-automation` tasks via Rube MCP (Composio).
