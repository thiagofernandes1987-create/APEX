...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.ip2location-io-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: ip2location-io-automation
...
what_if_fails: >
  FALLBACK: If Ip2Location Io Automation cannot complete, provide partial results with
...
Stub for the `ip2location-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ip2location-io-automation` tasks via Rube MCP (Composio).
