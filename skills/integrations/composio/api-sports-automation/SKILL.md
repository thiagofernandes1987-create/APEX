...
what_if_fails: >
  FALLBACK: If Api Sports Automation cannot complete, provide partial results with
...
Stub for the `api-sports-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `api-sports-automation` tasks via Rube MCP (Composio).
