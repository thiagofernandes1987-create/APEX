...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.google_search_console-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: google_search_console-automation
...
what_if_fails: >
  FALLBACK: If Google_Search_Console Automation cannot complete, provide partial results with
...
Stub for the `google_search_console-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `google_search_console-automation` tasks via Rube MCP (Composio).
