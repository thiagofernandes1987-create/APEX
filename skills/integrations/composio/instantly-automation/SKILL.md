...
what_if_fails: >
  FALLBACK: If Instantly cannot complete, provide partial results with
...
Stub for the `instantly-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `instantly-automation` tasks via Rube MCP (Composio).
