...
toolkit: emaillistverify-automation
...
what_if_fails: >
  FALLBACK: If Emaillistverify Automation cannot complete, provide partial results with
...
Stub for the `emaillistverify-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `emaillistverify-automation` tasks via Rube MCP (Composio).
