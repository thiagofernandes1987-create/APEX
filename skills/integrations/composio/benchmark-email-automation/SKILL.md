...
toolkit: benchmark-email-automation
...
what_if_fails: >
  FALLBACK: If Benchmark Email Automation cannot complete, provide partial results with
...
Stub for the `benchmark-email-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `benchmark-email-automation` tasks via Rube MCP (Composio).
