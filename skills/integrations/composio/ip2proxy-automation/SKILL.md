...
what_if_fails: >
  FALLBACK: If Ip2Proxy Automation cannot complete, provide partial results with
...
Stub for the `ip2proxy-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ip2proxy-automation` tasks via Rube MCP (Composio).
