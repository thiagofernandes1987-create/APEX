...
what_if_fails: >
  FALLBACK: If Eventbrite cannot complete, provide partial results with
...
Stub for the `eventbrite-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `eventbrite-automation` tasks via Rube MCP (Composio).
