...
what_if_fails: >
  FALLBACK: If Thanks Io Automation cannot complete, provide partial results with
...
Stub for the `thanks-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `thanks-io-automation` tasks via Rube MCP (Composio).
