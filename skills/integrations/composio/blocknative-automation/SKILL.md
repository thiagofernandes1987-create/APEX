...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.blocknative-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: blocknative-automation
...
what_if_fails: >
  FALLBACK: If Blocknative Automation cannot complete, provide partial results with
...
Stub for the `blocknative-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `blocknative-automation` tasks via Rube MCP (Composio).
