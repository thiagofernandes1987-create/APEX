...
what_if_fails: >
  FALLBACK: If Lemlist cannot complete, provide partial results with
...
Stub for the `lemlist-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `lemlist-automation` tasks via Rube MCP (Composio).
