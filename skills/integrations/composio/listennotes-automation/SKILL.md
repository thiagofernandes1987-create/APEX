...
toolkit: listennotes-automation
...
what_if_fails: >
  FALLBACK: If Listennotes Automation cannot complete, provide partial results with
...
Stub for the `listennotes-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `listennotes-automation` tasks via Rube MCP (Composio).
