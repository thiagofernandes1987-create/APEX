...
what_if_fails: >
  FALLBACK: If Dungeon Fighter Online Automation cannot complete, provide partial results with
...
Stub for the `dungeon-fighter-online-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `dungeon-fighter-online-automation` tasks via Rube MCP (Composio).
