...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.sms-alert-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: sms-alert-automation
...
what_if_fails: >
  FALLBACK: If Sms Alert Automation cannot complete, provide partial results with
...
Stub for the `sms-alert-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `sms-alert-automation` tasks via Rube MCP (Composio).
