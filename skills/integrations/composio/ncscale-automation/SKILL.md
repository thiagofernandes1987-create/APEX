...
what_if_fails: >
  FALLBACK: If Ncscale Automation cannot complete, provide partial results with
...
Stub for the `ncscale-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ncscale-automation` tasks via Rube MCP (Composio).
