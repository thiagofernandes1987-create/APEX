...
what_if_fails: >
  FALLBACK: If Ai Ml Api Automation cannot complete, provide partial results with
...
Stub for the `ai-ml-api-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ai-ml-api-automation` tasks via Rube MCP (Composio).
