...
toolkit: async-interview-automation
...
what_if_fails: >
  FALLBACK: If Async Interview Automation cannot complete, provide partial results with
...
Stub for the `async-interview-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `async-interview-automation` tasks via Rube MCP (Composio).
