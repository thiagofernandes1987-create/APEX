...
toolkit: semanticscholar-automation
...
what_if_fails: >
  FALLBACK: If Semanticscholar Automation cannot complete, provide partial results with
...
Stub for the `semanticscholar-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `semanticscholar-automation` tasks via Rube MCP (Composio).
