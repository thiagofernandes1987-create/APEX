...
toolkit: cloudconvert-automation
...
what_if_fails: >
  FALLBACK: If Cloudconvert Automation cannot complete, provide partial results with
...
Stub for the `cloudconvert-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `cloudconvert-automation` tasks via Rube MCP (Composio).
