...
what_if_fails: >
  FALLBACK: If Findymail Automation cannot complete, provide partial results with
...
Stub for the `findymail-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `findymail-automation` tasks via Rube MCP (Composio).
