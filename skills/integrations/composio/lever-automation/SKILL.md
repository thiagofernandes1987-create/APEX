...
what_if_fails: >
  FALLBACK: If Lever cannot complete, provide partial results with
...
Stub for the `lever-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `lever-automation` tasks via Rube MCP (Composio).
