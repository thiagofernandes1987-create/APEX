...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.mails-so-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: mails-so-automation
...
what_if_fails: >
  FALLBACK: If Mails So Automation cannot complete, provide partial results with
...
Stub for the `mails-so-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `mails-so-automation` tasks via Rube MCP (Composio).
