...
toolkit: timelinesai-automation
...
what_if_fails: >
  FALLBACK: If Timelinesai Automation cannot complete, provide partial results with
...
Stub for the `timelinesai-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `timelinesai-automation` tasks via Rube MCP (Composio).
