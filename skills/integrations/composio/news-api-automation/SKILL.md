...
what_if_fails: >
  FALLBACK: If News Api Automation cannot complete, provide partial results with
...
Stub for the `news-api-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `news-api-automation` tasks via Rube MCP (Composio).
