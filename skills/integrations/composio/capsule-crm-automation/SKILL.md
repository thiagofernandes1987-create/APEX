...
toolkit: capsule-crm-automation
...
what_if_fails: >
  FALLBACK: If Capsule cannot complete, provide partial results with
...
Stub for the `capsule-crm-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `capsule-crm-automation` tasks via Rube MCP (Composio).
