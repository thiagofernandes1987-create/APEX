...
toolkit: census-bureau-automation
...
what_if_fails: >
  FALLBACK: If Census Bureau Automation cannot complete, provide partial results with
...
Stub for the `census-bureau-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `census-bureau-automation` tasks via Rube MCP (Composio).
