...
toolkit: identitycheck-automation
...
what_if_fails: >
  FALLBACK: If Identitycheck Automation cannot complete, provide partial results with
...
Stub for the `identitycheck-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `identitycheck-automation` tasks via Rube MCP (Composio).
