...
toolkit: optimoroute-automation
...
what_if_fails: >
  FALLBACK: If Optimoroute Automation cannot complete, provide partial results with
...
Stub for the `optimoroute-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `optimoroute-automation` tasks via Rube MCP (Composio).
