...
toolkit: ravenseotools-automation
...
what_if_fails: >
  FALLBACK: If Ravenseotools Automation cannot complete, provide partial results with
...
Stub for the `ravenseotools-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ravenseotools-automation` tasks via Rube MCP (Composio).
