...
what_if_fails: >
  FALLBACK: If Tapfiliate Automation cannot complete, provide partial results with
...
Stub for the `tapfiliate-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `tapfiliate-automation` tasks via Rube MCP (Composio).
