...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.twelve-data-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: twelve-data-automation
...
what_if_fails: >
  FALLBACK: If Twelve Data Automation cannot complete, provide partial results with
...
Stub for the `twelve-data-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `twelve-data-automation` tasks via Rube MCP (Composio).
