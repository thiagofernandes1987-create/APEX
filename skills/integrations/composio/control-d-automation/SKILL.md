...
what_if_fails: >
  FALLBACK: If Control D Automation cannot complete, provide partial results with
...
Stub for the `control-d-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `control-d-automation` tasks via Rube MCP (Composio).
