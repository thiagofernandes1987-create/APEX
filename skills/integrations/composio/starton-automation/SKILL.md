...
what_if_fails: >
  FALLBACK: If Starton Automation cannot complete, provide partial results with
...
Stub for the `starton-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `starton-automation` tasks via Rube MCP (Composio).
