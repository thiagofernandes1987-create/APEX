...
what_if_fails: >
  FALLBACK: If Auth0 Automation cannot complete, provide partial results with
...
Stub for the `auth0-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `auth0-automation` tasks via Rube MCP (Composio).
