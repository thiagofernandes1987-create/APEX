...
what_if_fails: >
  FALLBACK: If Espocrm Automation cannot complete, provide partial results with
...
Stub for the `espocrm-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `espocrm-automation` tasks via Rube MCP (Composio).
