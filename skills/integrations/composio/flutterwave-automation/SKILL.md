...
toolkit: flutterwave-automation
...
what_if_fails: >
  FALLBACK: If Flutterwave Automation cannot complete, provide partial results with
...
Stub for the `flutterwave-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `flutterwave-automation` tasks via Rube MCP (Composio).
