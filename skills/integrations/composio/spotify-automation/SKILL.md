...
what_if_fails: >
  FALLBACK: If Spotify cannot complete, provide partial results with
...
Stub for the `spotify-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `spotify-automation` tasks via Rube MCP (Composio).
