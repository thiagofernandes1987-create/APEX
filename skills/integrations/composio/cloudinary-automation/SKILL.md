...
what_if_fails: >
  FALLBACK: If Cloudinary cannot complete, provide partial results with
...
Stub for the `cloudinary-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `cloudinary-automation` tasks via Rube MCP (Composio).
