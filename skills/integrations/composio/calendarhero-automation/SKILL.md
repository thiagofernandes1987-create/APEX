...
toolkit: calendarhero-automation
...
what_if_fails: >
  FALLBACK: If Calendarhero Automation cannot complete, provide partial results with
...
Stub for the `calendarhero-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `calendarhero-automation` tasks via Rube MCP (Composio).
