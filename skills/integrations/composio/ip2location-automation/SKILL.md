...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.ip2location-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: ip2location-automation
...
what_if_fails: >
  FALLBACK: If Ip2Location Automation cannot complete, provide partial results with
...
Stub for the `ip2location-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `ip2location-automation` tasks via Rube MCP (Composio).
