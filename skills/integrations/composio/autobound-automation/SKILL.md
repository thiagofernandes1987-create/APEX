...
what_if_fails: >
  FALLBACK: If Autobound Automation cannot complete, provide partial results with
...
Stub for the `autobound-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `autobound-automation` tasks via Rube MCP (Composio).
