...
toolkit: better-proposals-automation
...
what_if_fails: >
  FALLBACK: If Better Proposals Automation cannot complete, provide partial results with
...
Stub for the `better-proposals-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `better-proposals-automation` tasks via Rube MCP (Composio).
