...
what_if_fails: >
  FALLBACK: If Toggl cannot complete, provide partial results with
...
Stub for the `toggl-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `toggl-automation` tasks via Rube MCP (Composio).
