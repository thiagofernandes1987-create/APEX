...
toolkit: productlane-automation
...
what_if_fails: >
  FALLBACK: If Productlane Automation cannot complete, provide partial results with
...
Stub for the `productlane-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `productlane-automation` tasks via Rube MCP (Composio).
