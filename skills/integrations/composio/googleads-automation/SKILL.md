...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.googleads-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: googleads-automation
...
output_schema:
  - name: report
    type: string
    description: "Analysis report or summary from googleads automation"
what_if_fails: >
  FALLBACK: If Googleads Automation cannot complete, provide partial results with
...
Stub for the `googleads-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `googleads-automation` tasks via Rube MCP (Composio).
