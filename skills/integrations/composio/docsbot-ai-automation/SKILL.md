...
what_if_fails: >
  FALLBACK: If Docsbot Ai Automation cannot complete, provide partial results with
...
Stub for the `docsbot-ai-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `docsbot-ai-automation` tasks via Rube MCP (Composio).
