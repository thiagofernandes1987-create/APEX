...
what_if_fails: >
  FALLBACK: If Addresszen Automation cannot complete, provide partial results with
...
Stub for the `addresszen-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `addresszen-automation` tasks via Rube MCP (Composio).
