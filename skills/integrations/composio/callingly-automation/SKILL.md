...
what_if_fails: >
  FALLBACK: If Callingly Automation cannot complete, provide partial results with
...
Stub for the `callingly-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `callingly-automation` tasks via Rube MCP (Composio).
