...
what_if_fails: >
  FALLBACK: If Fixer Automation cannot complete, provide partial results with
...
Stub for the `fixer-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `fixer-automation` tasks via Rube MCP (Composio).
