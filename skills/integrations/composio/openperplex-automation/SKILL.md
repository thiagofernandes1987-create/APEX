...
toolkit: openperplex-automation
...
what_if_fails: >
  FALLBACK: If Openperplex Automation cannot complete, provide partial results with
...
Stub for the `openperplex-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `openperplex-automation` tasks via Rube MCP (Composio).
