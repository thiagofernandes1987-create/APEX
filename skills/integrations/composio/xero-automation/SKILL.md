...
output_schema:
  - name: result
    type: string
    description: "Primary output from Xero"
what_if_fails: >
  FALLBACK: If Xero cannot complete, provide partial results with
...
Stub for the `xero-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `xero-automation` tasks via Rube MCP (Composio).
