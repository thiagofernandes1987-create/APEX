...
requires:
  mcp: [rube]
executor: HYBRID
skill_id: integrations.composio.melo-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: melo-automation
...
what_if_fails: >
  FALLBACK: If Melo Automation cannot complete, provide partial results with
...
Stub for the `melo-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `melo-automation` tasks via Rube MCP (Composio).
