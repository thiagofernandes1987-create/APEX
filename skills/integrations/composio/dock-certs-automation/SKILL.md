...
what_if_fails: >
  FALLBACK: If Dock Certs Automation cannot complete, provide partial results with
...
Stub for the `dock-certs-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `dock-certs-automation` tasks via Rube MCP (Composio).
