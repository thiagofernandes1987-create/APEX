...
toolkit: bookingmood-automation
...
what_if_fails: >
  FALLBACK: If Bookingmood Automation cannot complete, provide partial results with
...
Stub for the `bookingmood-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `bookingmood-automation` tasks via Rube MCP (Composio).
