...
Stub for the `zoho_invoice-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `zoho_invoice-automation` tasks via Rube MCP (Composio).
