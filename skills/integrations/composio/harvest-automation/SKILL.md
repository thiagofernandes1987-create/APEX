...
what_if_fails: >
  FALLBACK: If Harvest cannot complete, provide partial results with
...
Stub for the `harvest-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `harvest-automation` tasks via Rube MCP (Composio).
