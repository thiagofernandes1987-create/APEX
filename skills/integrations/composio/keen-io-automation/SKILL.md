...
what_if_fails: >
  FALLBACK: If Keen Io Automation cannot complete, provide partial results with
...
Stub for the `keen-io-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `keen-io-automation` tasks via Rube MCP (Composio).
