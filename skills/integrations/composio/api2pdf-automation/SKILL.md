...
what_if_fails: >
  FALLBACK: If Api2Pdf Automation cannot complete, provide partial results with
...
Stub for the `api2pdf-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `api2pdf-automation` tasks via Rube MCP (Composio).
