...
what_if_fails: >
  FALLBACK: If Botstar Automation cannot complete, provide partial results with
...
Stub for the `botstar-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `botstar-automation` tasks via Rube MCP (Composio).
