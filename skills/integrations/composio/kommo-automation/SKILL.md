...
requires:
  mcp:
    - rube
executor: HYBRID
skill_id: integrations.composio.kommo-automation
status: ADOPTED
security: {level: standard, pii: false, approval_required: false}
extends: integrations.composio.meta
toolkit: kommo-automation
...
what_if_fails: >
  FALLBACK: If Kommo cannot complete, provide partial results with
...
Stub for the `kommo-automation` toolkit in the Composio integration ecosystem.
Extends `integrations.composio.meta` — see the meta-skill for full protocol.

## When to Use
Use when automating `kommo-automation` tasks via Rube MCP (Composio).
