...
name: zeroize-audit
description: '''Detects missing zeroization of sensitive data in source code and identifies zeroization removed by compiler
  optimizations, with assembly-level analysis, and control-flow verification. Use for auditin'
version: v00.33.0
status: CANDIDATE
domain_path: engineering/programming/rust/zeroize-audit
anchors:
- zeroize
- audit
- detects
...
output_schema:
  ...
  description: 'Each run produces two outputs:


    1. **`final-report.md`** — Comprehensive markdown report (primary human-readable output)

    2. **`findings.json`** — Structured JSON matching `{baseDir}/schemas/output.jso'
what_if_fails:
