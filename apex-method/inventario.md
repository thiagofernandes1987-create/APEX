...
**v1.51.0 — transporte da federação: staging/ → commit.** `federation.publish_pack()` fecha a rotina de publicação: pacote → tier `staging/` da árvore de swap (nome canônico versionado) → `apex-method/federation/packs/` no clone local → **commit git** automático (push/PR continua ação do usuário). Endereçado pelo hash do CONHECIMENTO: republicar sem aprendizado novo = ALREADY_PUBLISHED (bug do exported_at volátil proliferando arquivos, achado no smoke e corrigido). Caminho de volta pós-`git pull`: `list_published()` (cada pacote verificado) → `import_from_repo(approved=True)` (H5 por pacote, local-vence, idempotente). CLI: publish / list-remote / import-repo. Suíte: benchmark 64/64.

**v1.50.0 — item 3 fechado + FEDERAÇÃO (executado com o próprio APEX: kernel roteou as duas tarefas, gate devolveu o bastão, passos completados com evidência).** (1) `rag_index.search(dim=...)` filtra pela matriz taxonômica (disciplina/especialização/modo — o TagRAG via as dims existentes); **`solid_prefix()`**: a convenção de prefixo estável p/ prompt-caching do provedor (bloco determinístico ~275 tk: overview + ambiente estável + governança; volátil sempre por último). (2) **`federation`**: pacotes de conhecimento assinados — SÓ o validado viaja (PROMOTED/promovável; poet e vacina fraca ficam de fora, provado em teste), procedência = cadeia de ledger do dispositivo re-verificada elo a elo, HMAC opcional (APEX_FED_KEY), import exige H5, adulterado → REJECTED antes de escrever, fusão LOCAL-VENCE idempotente (bug de assinatura `_counts` e de dedup de grants por ts achados e corrigidos no smoke). 49 scripts. Suíte: benchmark 63/63.
...
## v1.52.0 — Autópsia de segurança validada por reprodução (3 auditorias externas + 2 dores do autor)

Cada achado foi **reproduzido na máquina antes de qualquer correção** (regra do autor: "não aceite nada sem validar"). **CONFIRMADOS e corrigidos:** **SEC-001** (PoT lia segredos do env e escrevia arquivos fora do home → env higienizado por allowlist + CWD descartável), **SEC-002** (descendentes órfãos sobreviviam ao timeout → morte da árvore de processos: `taskkill /T` no Windows, `killpg` no POSIX), **SEC-003** (stdout ilimitado → teto de 1 MB + marcador `[OUTPUT_TRUNCATED]`), **SEC-004** (gates aceitavam `open(...,'w').write()` — builtin sem import → detecção de escrita em `guards.forge_load_gate` e `skill_scout`, leitura e `json.loads` seguem liberados), **SEC-005/006/007** em `repo_bridge` (escape por symlink → `realpath`+containment; redirect cross-repo → valida owner/repo, não só host; truncamento silencioso → lê `max_bytes+1` e recusa `REFUSED_OVERSIZE`), **UPD-001** (`menu update --apply` sobrescrevia o instalado antes de validar → agora transacional: staging → benchmark → swap atômico → rollback), **FUNC-001** (melhoria grande do Hamiltoniano virava `CRITICAL` por `abs(ΔH)` simétrico → penaliza só regressão `max(0, H−prev)`; a demo agora reporta `STABLE`). **USER-A (a dor nº1 do autor):** personas de auditoria eram equipadas com skills de **design visual** (extract-design-system, canvas, shadcn, ui-ux) porque o seed de equipamento concatenava o vocabulário genérico da persona ("system_**design**", "architecture") ao query gravitacional → agora o `equip_for` semeia **só pela TAREFA**; o architect de auditoria passou a atrair `brainstorming` e o critic `requesting-code-review`. **USER-B:** a magnitude "milhares de subprocessos/RAM" **não se reproduziu** (medido: ~38–80 spawns curtíssimos, pico ~5 processos simultâneos, ~233 MB); o smell real (interpretador novo por passo PoT) foi mitigado — a flag `-s` + env enxuto **derrubaram o benchmark de 140 s → 29 s**. Teste de regressão `autopsy_v152` tranca os 8 achados. Suítes: **benchmark 65/65 · evaluate 13/13 (100%) · scenario 7/7**.
